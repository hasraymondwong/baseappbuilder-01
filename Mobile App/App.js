    ...
    if(firebase.apps.length){
      ...
      firebase.database().ref(appConfig.expo.extra.firebaseMetaPath).once('value').then(function(snapshot) {
        ...
        if(snapshot.val().settings!=null){
          //AdMob settings
          Config.showBannerAds=snapshot.val().settings.adMob.showBannerAds||false;
          Config.showinterstitialAds=snapshot.val().settings.adMob.showinterstitialAds||false;
          Config.bannerID=snapshot.val().settings.adMob.bannerID||"";

          //LoginSettings
          Config.loginSetup={
            welcomeText: snapshot.val().settings.login.welcomeText||"",
            facebookLogin: snapshot.val().settings.login.facebookLogin||false, 
            facebookID: snapshot.val().settings.login.facebookID||"178511486175063",
            googleLogin: snapshot.val().settings.login.googleLogin||false, 
            googleIOSid: snapshot.val().settings.login.googleIOSid||"148773293873-o35mgo7q5ceea45v4fhd9uqivgtjlh4j.apps.googleusercontent.com",
            googleAndroidId: snapshot.val().settings.login.googleAndroidId||"419235345147-5ld8h97mhnk6qq257djds3bu1l9acfuu.apps.googleusercontent.com"
          }

          //PayPal settings
          Config.paypal={
            acceptPayments: snapshot.val().settings.paypal.acceptPayments||true,
            sandBoxMode: snapshot.val().settings.paypal.sandBoxMode||true,
            clientID: snapshot.val().settings.paypal.clientID||"Af_H2HSMUFkVQsDfIggWgobv-QK59pLOR6iX77TpEWLUN8ob0eBGCg48CBX1gcifFKUdu0YHRfyS6Tnl",
            secretKey: snapshot.val().settings.paypal.secretKey||"EHrmFLREuoQ7FMIEITEKckqydqhtQan07pIy0Uhc1TnNmmE33_xWfqlFoBXHg7gjuismQQaNoSzMLWIS",
            return_url: snapshot.val().settings.paypal.return_url||"https://envato.com/#products",
            cancel_url: snapshot.val().settings.paypal.cancel_url||"https://market.envato.com/",
            includeShippingInfo: snapshot.val().settings.paypal.includeShippingInfo||true,
            currency: snapshot.val().settings.paypal.currency||"$",
            state: snapshot.val().settings.paypal.state||"CA",
            country_code: snapshot.val().settings.paypal.country_code||"US",
            postal_code: snapshot.val().settings.paypal.postal_code||"95131",
            city: snapshot.val().settings.paypal.city||"San Jose",
          }

          //Order setup
          Config.sendToEmail= snapshot.val().settings.orders.sendToEmail||"contact@mobidonia.com"

          AdMobInterstitial.setAdUnitID(snapshot.val().settings.adMob.interstitialID||"");
      }else{
        //Defined from local
      }
