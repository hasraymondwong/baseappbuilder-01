...
export default class ListOfUsers extends Component {
  ...
  _keyExtractor = (item, index) => item.id+Date.now();
  ...
  */
  getUsers(){
    ...
    firebase.database().ref('/chats/'+firebase.auth().currentUser.uid).orderByChild("lastChat").on('value', function(snapshot) {
        ...
        alert("The lenght is : "+JSON.stringify(data.length))
        _this.setState({
             items:[],
             items:data.reverse(),
             itemsStore:data,
 ...
 renderItem(data){
   ...
   */
  openChat(item){
    //selectedUser:item.uid.indexOf('group_') !== -1?item.uid
    if((item.uid.indexOf('group_') !== -1)){
      // here also add the group of ids
      this.props.navigation.navigate('Chat', {chatID:item.uid,path:"messages/",groupChatIds:item.usersInTheChat});
    }else{
