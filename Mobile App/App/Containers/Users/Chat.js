...
componentDidMount(){
  ...
  setUpCurrentUser(user){   
   ...
   // User is signed in.
    if (user != null) {
      ...
      if(this.props.navigation.state.params.groupChatIds == null){
        _this.setState({
          avatar:user.photoURL != null?user.photoURL:"",
          name : user.displayName,
          userID:firebase.auth().currentUser.uid,
          chatID: firebase.auth().currentUser.uid > this.state.selectedUser?firebase.auth().currentUser.uid + this.state.selectedUser:this.state.selectedUser + firebase.auth().currentUser.uid,
          isLoggedIn:true,
  ...
  */ 
  ...
  sendPushNotification(message){
    ...
    api.post("",{ "to": this.state.selectedUserToken, "title":this.state.name,
    "body": message.text,"sound": "default"}, { headers: {'Accept': 'application/json',
    'Content-Type': 'application/json', 'Accept-Encoding': 'gzip, deflate'} }).then((result) => {
      if(result.ok){
        console.log("Ok");
       }else{
        console.log("Error");
